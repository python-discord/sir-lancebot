...
def test_addguess():
    # Contract: A guess is applied when its a valid guess. The rows and columns are within limits
    # and the guess is within a valid range
    ...
    assert cell_three.guess == 3
    testingGrid.add_guess("C1 2")
    assert cell_three.guess == 2
    assert testingGrid.add_guess("A2 2")
    assert testingGrid.add_guess("D2 2") is False
    assert testingGrid.add_guess("A4 2") is False
    assert testingGrid.add_guess("A2 4") is False
    assert testingGrid.add_guess("A2 0") is False
    assert testingGrid.add_guess("A2 -1") is False
