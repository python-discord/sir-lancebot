...
class NationDay(Cog):
    ...
    async def get_specific_country(self, country: str) -> Tuple[List[Tuple[str, str]], str]:
        ...
        if date:
            page, img = await self.get_country_info(self.http_session, country)
    ...
    async def get_country_info(self, client: ClientSession, country: str) -> Tuple[str, str]:
        """
        ...
        try:
            ...
            if iso_code is None:
                not_available_msg = f"Could not find information on this country."
                logger.warning(not_available_msg)
                return not_available_msg, white_flag
            async with client.get(url+iso_code) as resp:
    ...
    async def nationday(self, ctx: Context, param: str = "") -> None:
        ...
        # Check if country is present
            ...
            if page[0][0] is not None:
                ...
                param_list = [x.capitalize() for x in param_list]
                param = ' '.join(param_list)
                embed = discord.Embed(
                    title=f'{param} -> {date.capitalize()}'
                ).set_footer(text='Powered by the RESTCountries API.')
                await ImagePaginator.paginate(page, ctx, embed)

        else:
            await ctx.channel.send("Give appropriate country name Eg. 'United States of America'\
